package hx.nine.eleven.http.client.excutor;

import hx.nine.eleven.http.client.config.HttpRequestConfig;
import hx.nine.eleven.http.client.entity.HttpHeader;
import hx.nine.eleven.http.client.enums.HttpStatus;
import hx.nine.eleven.http.client.exception.HttpConnectException;
import hx.nine.eleven.http.client.response.HttpResponse;
...
import static hx.nine.eleven.http.client.enums.RequestMethodEnum.*;
...
public class HttpClientExcutor {
        ...
        switch (httpRequestConfig.getMethod()){
            ...
            default:
                throw new HttpConnectException(String.format("request method is not in method list,method :[s%], methodList:[s%]",
                        httpRequestConfig.getMethod(), values()));
        }
        return null;
    }
    ...
    private <T> HttpResponse<T> doGet(HttpRequestConfig httpRequestConfig, Class<T> classzz)
        ...
        URI uri = setGetUrl(url, (Map<String, Object>) httpRequestConfig.getParams());
        // 创建http GET请求
    ...
    private <T> HttpResponse<T> doPost(HttpRequestConfig httpRequestConfig,
        ...
        setParam((Map<String, Object>) params,httpPost,httpRequestConfig.isForm());
        // 配置请求的超时设置
