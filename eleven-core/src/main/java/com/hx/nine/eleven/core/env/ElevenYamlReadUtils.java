 ...
 * @date 2023-03-24
 */
public class ElevenYamlReadUtils {

	private final static Logger LOGGER = LoggerFactory.getLogger(ElevenYamlReadUtils.class);

	private static LinkedHashMap valueMap;

	private ElevenYamlReadUtils() {
		valueMap = new LinkedHashMap();
	}

	public static ElevenYamlReadUtils build() {
	...
	public <T> List<T> getProperties(Class<T> propertiesClass, String prefix) {
		...
		if (!Optional.ofNullable(values).isPresent()) {
			...
			if (propertyListMap.size() > 0){
				...
				DefaultElevenApplicationContext.build().addBean(ConstantType.CONDITIONAL_ON_NEST_PROPERTITES_BEAN, nextPropertiesBeans,true);
			}
			// 赋值
	...
	public ElevenYamlReadUtils addProperties(String key, String value){
	...
	private void checkConfigUrl(String configUrl){
		...
		if (lastIndex != index || configUrl.indexOf("*.") == -1 || lastFile > lastIndex){
			// 抛出异常，应用停止
			throw new ElevenApplicationRunException("config url 路径中只能配置一个 ‘*’ ，且只能模糊匹配文件，如：/home/app/config/*.yml");
		}
	}
	...
	private final static class Signle {
		public final static ElevenYamlReadUtils INSTANCE = new ElevenYamlReadUtils();
	}
}
