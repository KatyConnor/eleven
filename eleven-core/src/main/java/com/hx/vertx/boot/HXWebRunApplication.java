...
import com.hx.vertx.boot.core.context.ClassPathBeanDefinitionScanner;
...
public class HXWebRunApplication{
  ...
  private final static Logger LOGGER = LoggerFactory.getLogger(HXWebRunApplication.class);
  ...
  public static void run(String[] args) {
    ...
    if (StringUtils.isNotEmpty(properties)){
      ...
      HXVertxYamlReadUtils.build().addProperties(prop[0],prop[1]).readYamlConfiguration(prop[1]);
    }else {
      HXVertxYamlReadUtils.build().readYamlConfiguration(null);
    }
    // 初始化bean容器，和上下文HXVertxApplicationContext
    ClassPathBeanDefinitionScanner.build().initClass();
    LOGGER.info("容器初始化完成！");
    VertxApplicationProperties vertxApplicationProperties = DefaultVertxApplicationContext.build().getProperties(VertxApplicationProperties.class);
    DeploymentOptions deploymentOptions = new DeploymentOptions();
    deploymentOptions.setWorkerPoolSize(vertxApplicationProperties.getWorkerPoolSize());  // 默认线程
