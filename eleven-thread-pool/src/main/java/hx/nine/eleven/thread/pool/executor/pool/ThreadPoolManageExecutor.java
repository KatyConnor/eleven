...
import hx.nine.eleven.thread.pool.executor.factory.AbstractThreadFactory;
import hx.nine.eleven.thread.pool.executor.StringUtils;
import hx.nine.eleven.thread.pool.executor.factory.DefaultThreadFactory;
import hx.nine.eleven.thread.pool.executor.fiber.FiberPoolExecutor;
...
public class ThreadPoolManageExecutor{
   ...
   public FiberPoolExecutor getFiberPoolExecutor(String threadGroupName){
       ...
       return fiberPoolExecutor;
//       throw new ThreadExecuteException(hx.nine.eleven.commons.utils.StringUtils.format("没有查找到线程池[{}]对应的协程执行器",groupName));
    }
