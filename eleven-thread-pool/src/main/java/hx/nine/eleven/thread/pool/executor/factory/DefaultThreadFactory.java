package hx.nine.eleven.thread.pool.executor.factory;
...
public class DefaultThreadFactory  extends AbstractThreadFactory  {
    ...
    public Thread newThread(Runnable r) {
        ...
        poolName.append("-").append(this.getThreadNamePrefix()).append("-")
                .append(this.threadGroupMap.get(this.getThreadGroup().getName()).getAndIncrement());
        Thread thread = new Thread(this.getThreadGroup(), MDCThreadUtil.wrap(r,
                MDC.getCopyOfContextMap()),  poolName.toString() , 0);
        if (thread.isDaemon()) {
