...
import com.hx.eleven.ftpserver.clienttests.ClientTestTemplate;
...
public class MinaClientAuthTest extends SSLTestTemplate {
    ...
    public void testCommandChannel() throws Exception {
        TestCase.assertTrue(client.login(ClientTestTemplate.ADMIN_USERNAME, ClientTestTemplate.ADMIN_PASSWORD));
        TestCase.assertTrue(FTPReply.isPositiveCompletion(client.noop()));
    }
    ...
    public void testClientCertificates() throws Exception {
        ...
        TestCase.assertEquals(1, session.getClientCertificates().length);

        X509Certificate cert = (X509Certificate) session
                .getClientCertificates()[0];

        TestCase.assertTrue(cert.getSubjectDN().toString().contains("ftpclient"));
    }
